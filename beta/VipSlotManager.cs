...
private int MaxPlayerCount; //xxxxxxxxxxxx
private int playerCurrentlyOnline; //xxxxxxxxxxxx
private int playerCurrentlyOnlineT1; //xxxxxxxxxxxx
private int playerCurrentlyOnlineT2; //xxxxxxxxxxxx
...
private string lastBF3disconnectionPlayername = String.Empty;
...
private DateTime lastBF3disconnectionTime = DateTime.UtcNow;
...
private enumBoolYesNo SettingKeepPluginEnabled;
private enumBoolYesNo SettingAggressiveJoinKickAbuse;
private enumBoolYesNo SettingAggressiveJoinFullServer;
private int SettingAggressiveJoinMinRealPlayers;	//xxxxxxxxxxxxxxxxxxx
...
public VipSlotManager() {
	...
	this.playerCurrentlyOnline = 0;
	this.playerCurrentlyOnlineT1 = 0;
	this.playerCurrentlyOnlineT2 = 0;
	...
	this.SettingKeepPluginEnabled = enumBoolYesNo.Yes;
	this.SettingAggressiveJoinKick = "%player% got disconnected to make room for !VIP on full server";
	this.SettingAggressiveJoinMsg = "%player% welcome back! You got KICKED randomly because a !VIP joined on full server :/";
	this.SettingAggressiveJoinMsgType = "Private Yell and Private Say";
	this.SettingAggressiveJoinKickAbuse = enumBoolYesNo.Yes;
	this.SettingAggressiveJoinFullServer = enumBoolYesNo.Yes;
	this.SettingAggressiveJoinMinRealPlayers = 0;	// xxxxxxxxxxxxxxx
...
[VdSk]LmaA-aD - Sponsoring BF3 & BFH Gameserver for testing Plugin
<p></p><p></p>
<h2>VIP Slot Manager [BF3,  BF4,  BFH,  BC2] [BETA]</h2>
...
- Fix: Website compatibility to php 5.6 / 7.0 / 7.1 / 7.2 / 7.3. New features and filters for better workflow<br/>
</blockquote><br>
...
public List<CPluginVariable> GetDisplayPluginVariables() {
	...
	lstReturn.Add(new CPluginVariable("8. Server - Aggressive Join for VIPs|Enable Aggressive Join on full Gameserver only", typeof(enumBoolYesNo), this.SettingAggressiveJoinFullServer));
	lstReturn.Add(new CPluginVariable("8. Server - Aggressive Join for VIPs|Disable Aggressive Join if less than # of real players are on server", this.SettingAggressiveJoinMinRealPlayers.GetType(), this.SettingAggressiveJoinMinRealPlayers));	// xxxxxxxxxxxxx
	...
	lstReturn.Add(new CPluginVariable("9. Advanced Settings|Keep Plugin enabled (auto enable if it has been disabled)", typeof(enumBoolYesNo), this.SettingKeepPluginEnabled));
	return lstReturn;
}
...
public void SetPluginVariable(String strVariable, String strValue) {
	...
	if (Regex.Match(strVariable, @"Debug Level").Success) {
		...
		if ((strValue.Length >= 3) && (strValue.Length <= 120)) {
			this.SettingInfoVip1 = strValue.Replace(System.Environment.NewLine, "");
		} else {
			ConsoleError("Invalid value for First Info Message (min. 3 and max. 120 chararcters)");
		}
		...
		if (strValue.Length <= 120) { this.SettingVipSpawnMsg2 = strValue.Replace(System.Environment.NewLine, ""); }
	} else if (Regex.Match(strVariable, @"Third VIP Spawn Message").Success) {
		if (strValue.Length <= 120) { this.SettingVipSpawnMsg3 = strValue.Replace(System.Environment.NewLine, ""); }
	} else if (Regex.Match(strVariable, @"VIP Spawn Yell").Success) {
		if (strValue.Length <= 120) { this.SettingVipSpawnYell = strValue; }
		...
		if (strValue.Length <= 120) { this.SettingVipExp = (enumBoolYesNo)Enum.Parse(typeof(enumBoolYesNo), strValue); }
		...
		if (strValue.Length <= 120) { this.SettingVipExpMsg2 = strValue.Replace(System.Environment.NewLine, ""); }
	} else if (Regex.Match(strVariable, @"Slot Expired Yell").Success) {
		if (strValue.Length <= 120) { this.SettingVipExpYell = strValue; }
		...
		if ((enumBoolYesNo)Enum.Parse(typeof(enumBoolYesNo), strValue) == enumBoolYesNo.Yes) {
			ConsoleWrite("'Aggressive Join' server setting enabled. Close on round end (less than 150 tickets) it will be disabled to keep as many players as possible on the server. On next round it will be enabled automatically. This feature works for the following game modes: ConquestLarge, ConquestSmall, TDM and Chainlink.");
		}
		...
		if (strValue.Length <= 120) { this.SettingAggressiveJoinMsg = strValue; }
		...
		if (strValue.Length <= 120) { this.SettingAggressiveJoinKickAbuseMsg = strValue; }
	} else if (Regex.Match(strVariable, @"Send Private Msg as:").Success) {
		this.SettingAggressiveJoinKickAbuseMsgType = strValue;
	} else if (Regex.Match(strVariable, @"Enable Aggressive Join on full Gameserver only").Success) {
		ConsoleWrite("'Aggressive Join' will be enabled automatically on full Gameserver only (less than 3 slots are free for real players). Otherwise the 'Aggressive Join' will be disabled temporary.");
		this.SettingAggressiveJoinFullServer = (enumBoolYesNo)Enum.Parse(typeof(enumBoolYesNo), strValue);
	} else if (Regex.Match(strVariable, @"Disable Aggressive Join if less than # of real players are on server").Success) {	// xxxxxxxxxxxxx
		if (this.SettingAggressiveJoinFullServer == enumBoolYesNo.Yes) {
			int tmpMinRealPlayers = 0;
			int.TryParse(strValue, out tmpMinRealPlayers);
			if (tmpMinRealPlayers >= 0 && tmpMinRealPlayers < 99) {
				this.SettingAggressiveJoinMinRealPlayers = tmpMinRealPlayers;
			} else {
				this.SettingAggressiveJoinMinRealPlayers = 3;
				ConsoleError("Invalid value. It must be a number between 0 and 99. (e.g.: 60)");
			}
		}
	...
	} else if (Regex.Match(strVariable, @"Keep Plugin enabled").Success) {
		this.SettingKeepPluginEnabled = (enumBoolYesNo)Enum.Parse(typeof(enumBoolYesNo), strValue);
	}
}
...
public void OnPluginLoaded(String strHostName, String strPort, String strPRoConVersion) {
	...
	// keep plugin enabled thread
	Thread KeepEnabledWorker1 = new Thread(new ThreadStart(delegate() {
		while (1 < 2) {
			Thread.Sleep(180000);
			if ((!this.fIsEnabled) && (this.SettingKeepPluginEnabled == enumBoolYesNo.Yes)) {
				if ((this.SettingStrSqlHostname != String.Empty) && (this.SettingStrSqlPort != String.Empty) && (this.SettingStrSqlDatabase != String.Empty) && (this.SettingStrSqlUsername != String.Empty) && (this.SettingStrSqlPassword != String.Empty)) {
					DebugWrite("[Task] [KeepPluginEnabled] Enable Plugin...", 5);
					this.ExecuteCommand("procon.protected.plugins.enable", "VipSlotManager", "True");
				}
			}
		}
	}));
	KeepEnabledWorker1.IsBackground = true;
	KeepEnabledWorker1.Name = "keepenabledworker1";
	KeepEnabledWorker1.Start();
}
...
public void OnPluginEnable() {
	...
	this.SyncCounter = 999;
	this.DBCleanerCounter = (new Random().Next(960, 990));
	this.lastBF3disconnectionPlayername = String.Empty;
...
public void OnPluginDisable() {
	...
	if (this.SettingKeepPluginEnabled == enumBoolYesNo.Yes) {
		if ((this.SettingStrSqlHostname != String.Empty) && (this.SettingStrSqlPort != String.Empty) && (this.SettingStrSqlDatabase != String.Empty) && (this.SettingStrSqlUsername != String.Empty) && (this.SettingStrSqlPassword != String.Empty)) {
			DebugWrite("[Tast] [KeepPluginEnabled] Plugin will be enabled in few minutes (Plugin setting: 'Keep Plugin enabled: YES').", 3);
		}
	}
}
		...
		if (CPlayerSubset.PlayerSubsetType.All == subset.Subset) {
			...
			foreach (CPlayerInfo playerinfo in players) {
				if (playerinfo.TeamID == 1) {
					this.playerCurrentlyOnlineT1++;
				} else if (playerinfo.TeamID == 2) {
					this.playerCurrentlyOnlineT2++;
				}
				if (this.SqlVipsActive.ContainsKey(playerinfo.SoldierName)) { this.vipsCurrentlyOnline++; }
				this.playerCurrentlyOnline++;
...
public override void OnPlayerJoin(String soldierName) {
			...
			if (this.RoundTempVips.Contains(soldierName)) {
				...
				if (this.AggressiveVips[soldierName] >= this.SettingAggressiveJoinKickAbuseMax) {
					...
					if ((this.AggressiveVips[soldierName] >= 2) && (this.SettingAdkatsLog == enumBoolYesNo.Yes) && (this.SettingAdkatsLogVipAggressiveJoinAbuse == enumBoolYesNo.Yes)) {
						// log to event logger xxxxxxxxxxxxxxxxxxxxxxxxxx  qqqqqqqqq
						this.ExecuteCommand("procon.protected.plugins.call", "EventLogger", "SqlLog", "VIP Abuse", soldierName, "Aggressive Join Abuse " + this.AggressiveVips[soldierName].ToString() + "/" + this.SettingAggressiveJoinKickAbuseMax.ToString() + " per round (blocked to use Aggressive Join Kick till next round)"); // xxxxxxxxxxxx qqqq adkats log / event logger log
	...
	// bf3 kicked for vip check
	if (tmp_playerTimestamp != -1) {
		if ((this.SettingGameType != "BF4") && (this.AggressiveJoin)) {
			if (this.MaxPlayerCount - this.playerCurrentlyOnline <= 1) {
				// maybe bf3 vip kick
				Thread BF3Worker1 = new Thread(new ThreadStart(delegate() {
					string tmp_lastBF3disconnectionPlayername = this.lastBF3disconnectionPlayername;
					DateTime tmp_lastBF3disconnectionTime = this.lastBF3disconnectionTime;
					string tmp_soldierName = soldierName;
					Thread.Sleep(1111);
					if ((tmp_lastBF3disconnectionPlayername.Length > 2) && (this.SettingAggressiveJoinMsg.Length > 2)) {
						if (((DateTime.UtcNow - tmp_lastBF3disconnectionTime).TotalSeconds) <= 4) {
							// player got kicked for vip
							DebugWrite(this.strBlack(tmp_lastBF3disconnectionPlayername) + " got KICKED to make room for VIP", 3);
							if (this.SettingAggressiveJoinKick == String.Empty) { this.ProconChat(tmp_lastBF3disconnectionPlayername + " got KICKED to make room for VIP"); }
							if ((this.SettingAdkatsLog == enumBoolYesNo.Yes) && (this.SettingAdkatsLogNonVipKick == enumBoolYesNo.Yes)) { this.AdkatsPlayerLog(tmp_lastBF3disconnectionPlayername, "got kicked to make room for VIP"); }
							this.SayMsg(this.SettingAggressiveJoinKick.Replace("%player%", tmp_lastBF3disconnectionPlayername).Replace("%online%", this.vipsCurrentlyOnline.ToString()).Replace("%total%", this.SqlVipsActive.Count.ToString())); 
							if (!this.kickedForVip.Contains(tmp_lastBF3disconnectionPlayername)) {this.kickedForVip.Add(tmp_lastBF3disconnectionPlayername); }
							this.LastAggressiveJoinKickTime = DateTime.UtcNow.AddMinutes(-5);
							if (!this.AggressiveVips.ContainsKey(tmp_soldierName)) { this.AggressiveVips.Add(tmp_soldierName, 0); }
							this.AggressiveVips[tmp_soldierName]++;
							DebugWrite("[OnJoin] [VIP] [AggressiveJoin] Valid VIP player " + this.strGreen(tmp_soldierName) + " joined with 'Aggressive Join Kick' (counter: " + this.AggressiveVips[tmp_soldierName].ToString() + "/" + this.SettingAggressiveJoinKickAbuseMax.ToString() + ").", 5);
						}
					}
				}));
				BF3Worker1.IsBackground = true;
				BF3Worker1.Name = "bf3worker1";
				BF3Worker1.Start();
			}
		}
	}
...
public override void OnServerInfo(CServerInfo serverInfo) {
	...
	this.MaxPlayerCount = serverInfo.MaxPlayerCount;
	if (!this.fIsEnabled) { return; }
...
public override void OnPlayerDisconnected(String playerName, String reason) {
	...
	if ((this.AggressiveJoin) && (reason == "PLAYER_KICKED")) {
		...
		this.SayMsg(this.SettingAggressiveJoinKick.Replace("%player%", playerName).Replace("%online%", this.vipsCurrentlyOnline.ToString()).Replace("%total%", this.SqlVipsActive.Count.ToString())); 
		if (!this.kickedForVip.Contains(playerName)) {this.kickedForVip.Add(playerName); }
		this.LastAggressiveJoinKickTime = DateTime.UtcNow;
	} else if ((this.SettingGameType != "BF4") && (this.AggressiveJoin)) {
		if (this.MaxPlayerCount - this.playerCurrentlyOnline <= 1) {
			// maybe bf3 vip kick, check in few sec
			this.lastBF3disconnectionPlayername = playerName;
			this.lastBF3disconnectionTime = DateTime.UtcNow;
		}
	}
				...
				if ((this.SqlVipsActive.ContainsKey(tmpPlayername)) || (this.RoundTempVips.Contains(tmpPlayername))) {
					...
					if (this.playerTeamID[tmpPlayername] > 0) {
						...
						if (this.MaxPlayerCount >= 10) {
							this.ExecuteCommand("procon.protected.send", "admin.listPlayers", "all");
							if (tmp_newteam == 1) {
								if (this.playerCurrentlyOnlineT1 >= this.MaxPlayerCount / 2) {
									// no room in t1...
									Thread ThreadWorker333 = new Thread(new ThreadStart(delegate() {
										Thread.Sleep(3333);
										if ((this.playerTeamID[tmpPlayername] > 0) && (this.playerTeamID[tmpPlayername] != tmp_newteam)) {
											// full server, can not switch...
											this.PlayerSayMsg(tmpPlayername, "Sorry " + tmpPlayername + ", there is no room in other team for switching. Try again later...");
										}
									}));
									ThreadWorker333.IsBackground = true;
									ThreadWorker333.Name = "threadworker333";
									ThreadWorker333.Start();
								}
							} else if (tmp_newteam == 2) {
								if (this.playerCurrentlyOnlineT2 >= this.MaxPlayerCount / 2) {
									// no room in t2...
									Thread ThreadWorker334 = new Thread(new ThreadStart(delegate() {
										Thread.Sleep(3333);
										if ((this.playerTeamID[tmpPlayername] > 0) && (this.playerTeamID[tmpPlayername] != tmp_newteam)) {
											// full server, can not switch...
											this.PlayerSayMsg(tmpPlayername, "Sorry " + tmpPlayername + ", there is no room in other team for switching. Try again later...");
									...
									ThreadWorker334.IsBackground = true;
									ThreadWorker334.Name = "threadworker334";
									ThreadWorker334.Start();
								}
							}
						}
					}
					...
					try {
						if (Connection.State == ConnectionState.Open) {
							try {
								...
								SQL = "SELECT `playername`, `status`, " + tmp_queryGuid + "TIMESTAMPDIFF(SECOND,'1970-01-01',timestamp) AS timestamp FROM `vsm_vips` WHERE gametype = '" + this.SettingGameType + "' AND servergroup = '" + this.SettingStrSqlServerGroup + "' AND status in ('active', 'adding', 'expired', 'deleting', 'removing') LIMIT 1500";
								using (MySqlCommand MyCommand = new MySqlCommand(SQL)) {
									DataTable resultTable = this.SQLquery(MyCommand);
									if (resultTable.Rows != null) {
	...
	if (this.SqlTableExist) {
		try {
			using (MySqlConnection Con = new MySqlConnection(this.SqlLogin())) {
				...
				try {
					if (Con.State == ConnectionState.Open) {
						string SQL = "DELETE FROM `vsm_vips` WHERE `vsm_vips`.status = 'inactive' AND (TIMESTAMPDIFF(DAY, timestamp, UTC_TIMESTAMP()) > 90) AND comment IS NULL AND gametype = '" + this.SettingGameType + "' AND servergroup = '" + this.SettingStrSqlServerGroup + "'";
...
private void AdkatsPlayerLog(String playername, String msg) {
	if ((playername.Length >= 3) && (msg.Length >= 3) && (this.NameGuidList.ContainsKey(playername))) { 
		if (this.AdkatsRunning) {
		...
		} else if (msg.Contains("Aggressive Join Abuse")) {
			this.ExecuteCommand("procon.protected.chat.write", "/vsm-adk-viab " + this.NameGuidList[playername] + " " +  playername);
		} else if (msg.Contains("room for VIP")) {
			this.ExecuteCommand("procon.protected.chat.write", "/vsm-adk-viki " + this.NameGuidList[playername] + " " +  playername);
		}
	}
}
...
public void updnow() {
	...
	bool tmp_EnableAggressive = true;
	if ((this.SettingAggressiveJoinFullServer == enumBoolYesNo.Yes) && (this.MaxPlayerCount > 0)) {
		if ((this.MaxPlayerCount - this.playerCurrentlyOnline > 5) || (this.MaxPlayerCount <= 20) || ((this.playerCurrentlyOnline < this.SettingAggressiveJoinMinRealPlayers) && this.SettingAggressiveJoinMinRealPlayers > 0)) {	// xxxxxxxxxxxxxxxxxxxxxxxxxxx
			// server not full
			tmp_EnableAggressive = false;
			if (this.AggressiveJoin) {
				// disable aggressive join
				DebugWrite("[Task] Aggressive Join for VIPs disabled (server not full).", 4);
				this.ProconChat("Aggressive Join for VIPs disabled (server not full).");
				this.ExecuteCommand("procon.protected.send", "reservedSlotsList.aggressiveJoin", "false");
			}
		} else if (((this.MaxPlayerCount - this.playerCurrentlyOnline < 3) && (this.MaxPlayerCount > 20)) || ((this.playerCurrentlyOnline > this.SettingAggressiveJoinMinRealPlayers) && this.SettingAggressiveJoinMinRealPlayers > 0)) {	// xxxxxxxxxxx
			// server full
			tmp_EnableAggressive = true;
			if ((this.SettingAggressiveJoin == enumBoolYesNo.No) && (!this.AggressiveJoin)) {
				// enable aggressive join
				DebugWrite("[Task] Aggressive Join for VIPs enabled (server is full)", 4);
				this.ProconChat("Aggressive Join for VIPs enabled (server is full)");
				this.ExecuteCommand("procon.protected.send", "reservedSlotsList.aggressiveJoin", "true");
			}
		}
	}
	
	// Enable / Disable Aggressive Join (close on round end)
	if ((this.SettingAggressiveJoin == enumBoolYesNo.Yes) && (tmp_EnableAggressive)) {
		if (this.AggressiveJoin) {
			if ((this.CurrentGameMode == "ConquestLarge0") || (this.CurrentGameMode == "ConquestSmall0") || (this.CurrentGameMode == "Chainlink0")) {
				if (this.ticketLoserTeam <= 150) {
					DebugWrite("[Task] Aggressive Join for VIPs disabled (less than 150 tickets remaining).", 4);
					this.ProconChat("Aggressive Join for VIPs disabled (less than 150 tickets remaining).");
					this.ExecuteCommand("procon.protected.send", "reservedSlotsList.aggressiveJoin", "false");
				}
